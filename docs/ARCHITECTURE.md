...
8. **`DataLoadRun` doesn't cover R2-fetch-layer failures.** A sha256 mismatch or missing manifest/parquet object is caught inside `r2.py`'s `fetch_council()`, before `load_council_spend()` — and therefore its `DataLoadRun.objects.create(...)` — is ever called. Today those failures are visible only as a `CommandError` in the invoking job's logs, not in the audit trail a `DataLoadRun` query would surface. Fine at pilot scale (one human running the command, reading its own output); worth a schema change (e.g. a fetch-layer audit row, or widening `DataLoadRun` to cover "attempted" not just "loaded") if/when a scheduled, unattended reload job (Phase 2's weekly diff) makes silent-in-logs failures a real risk.
9. **`list_councils()` exists now, unused until Phase 2's weekly reload job.** `spend/services/r2.py`'s `list_councils()` (`ListObjectsV2` on the `manifest/` prefix) was built alongside `fetch_council()` since the weekly diff-and-reload job will need it to discover which councils exist in R2 at all — not exercised by anything yet beyond its own test.
