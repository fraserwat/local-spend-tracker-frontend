...
- **Load strategy: idempotent full-replace per council**, not incremental upsert — the only correct approach given no transaction ID exists upstream. Delete all `SpendTransaction` rows for the council, `COPY` the new set into an unlogged staging table then `INSERT...SELECT` into place, update `CouncilCoverage`'s denormalized date fields. Re-running the loader twice must produce an identical end state with zero duplicates. Because delete+insert commits atomically, Postgres's MVCC guarantees a concurrent reader (someone browsing the Spend View mid-reload) sees either the fully-old or fully-new dataset, never a partial/empty state — this is a designed property, not incidental. Switched from `bulk_create` (1.47x slower per an earlier benchmark, originally kept for its simplicity at pilot scale) once the England-wide backfill made that gap worth taking.
...
10. ~~**Boundary/GeoJSON file count at England scale.**~~ — **Resolved.** All 296 English LADs are bundled into one `all-councils.geojson` (PR #36's idle-outline bundling, built for the London pilot, carried over unchanged): 5.1MB raw / 1.6MB gzipped, confirmed one request total regardless of council count — the "low-single-digit MB" estimate held.
...
12. **`pg_trgm` GIN index untuned for bursty delete+bulk-insert writes.** The same write pattern (bursty delete-then-bulk-insert per council) caused a real GitLab outage via GIN pending-list overflow. Migration `0003_beneficiary_name_trgm_fastupdate_off` sets `fastupdate = off` on `spend_spendtransaction_beneficiary_name_trgm`, removing that overflow failure mode entirely — but live England-scale reloads showed per-row insert cost growing with table size (Croydon: ~17,000 rows/sec on a near-empty table down to ~1,200 rows/sec at ~9M rows loaded), a real cost, not the "marginal" one originally assumed. Plausibly `fastupdate = off` doing more synchronous per-row GIN work as the index grows, still not confirmed. The full 274-council backfill (~94.6M rows) completed anyway at 8,000–10,500 rows/sec sustained, once paired with the `COPY` loader switch and a temporary `shared-cpu-4x` machine (both above) — root cause still open, revisit before the next order-of-magnitude jump in table size.
...
Three things only a real deploy against live traffic surfaced, none reproducible from `docker run` alone:
- **Fly's health checker doesn't send a `Host` header matching the public domain by default** — `ALLOWED_HOSTS` then 400s every check and the machine never registers healthy. Fixed with an explicit `[http_service.checks.headers] Host = "..."` override in `fly.toml`.
- **`gunicorn`'s sync worker only checks in with the arbiter between requests, not mid-response** — a single request that runs longer than `--timeout` gets its worker killed mid-stream, truncating the response with a *silent* HTTP 200 instead of an error. Measured live against Croydon's real 1,048,125-row dataset: the full 500,000-row CSV export cap takes just over 300s end to end on a shared-cpu-1x machine (~1,600 rows/sec — Neon is an external network hop, not localhost). `--timeout 600 --workers 3` gives real headroom; 2 workers was also too thin, since a multi-minute export legitimately holds one worker for its duration.
- **Scale-to-zero doesn't mix with multi-minute streaming responses.** With `auto_stop_machines`/`min_machines_running=0` (Fly's own template default), an in-flight CSV export was cut short mid-stream well under any worker timeout — Fly's "excess capacity" autostop doesn't appear to count an open streaming connection as active traffic. This also better matches the original cost assumption above (a small *always-on* machine, unlike Neon, which is deliberately designed to scale to zero) — `min_machines_running = 1`, `auto_stop_machines = 'off'`.
