...
12. **`pg_trgm` GIN index untuned for bursty delete+bulk-insert writes.** The same write pattern (bursty delete-then-bulk-insert per council) caused a real GitLab outage via GIN pending-list overflow. Migration `0003_beneficiary_name_trgm_fastupdate_off` sets `fastupdate = off` on `spend_spendtransaction_beneficiary_name_trgm`, removing that overflow failure mode entirely — but live England-scale reloads now show per-row insert cost growing with table size (Croydon: ~17,000 rows/sec on a near-empty table down to ~1,200 rows/sec at ~9M rows loaded), a real cost, not the "marginal" one originally assumed. Plausibly `fastupdate = off` doing more synchronous per-row GIN work as the index grows, not confirmed. Candidate fix: drop the index before a bulk load and rebuild after, rather than maintaining it row-by-row during the load — not yet done, degraded search is the tradeoff during the gap.
