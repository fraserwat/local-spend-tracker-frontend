...
- [ ] **Step 1 — Backend & API scaling.** Reference-data onboarding is bulk now (`onboard_english_councils`), not one-at-a-time. **ETL backfill done**: `reload_from_r2` loaded all 264 newly-onboarded councils R2 publishes -- 274 of 296 councils now have real spend data (the other 22 have no curated parquet in R2 at all). Load path switched from `bulk_create` to `COPY`-staging (see ARCHITECTURE's load-strategy section). Still open: a **generated (not hand-typed) `CouncilCoverage` fixture** (ARCHITECTURE Open risk #7) — 289 of 296 councils still have no coverage row, by design.
  *Verify:* a batch run onboards N councils in one pass (done); load throughput benchmarked and documented (done, see ARCHITECTURE Open risk #12); coverage fixture is generated, not hand-typed (not done).
