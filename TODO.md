...
- [ ] **Step 12 — Design refresh.** Current UI is functional, but looks "off". Before Phase 2 puts ~300 councils' worth of load on it:
  ...
  - **WCAG 2.2 AA audit** — contrast, keyboard nav, screen-reader labels are partially handled (`prefers-reduced-motion`, `aria-live`) but not formally checked end-to-end.
...
- [ ] **Step 1 — Backend & API scaling.** Reference-data onboarding is bulk now, not one-at-a-time: `onboard_english_councils` pulls GSS code + region for every English LAD straight from ONS Open Geography and diffs against what's already in the DB, replacing the old hand-written-migration-per-borough pattern. Still open: an **ETL backfill** for the 264 councils this added (`reload_from_r2` already knows how to pick each one up, deferred pending a decision on Neon cost/runtime at that volume). Also open: a **generated (not hand-typed) `CouncilCoverage` fixture** (ARCHITECTURE Open risk #7) — hand-transcription doesn't scale past the pilot, so 289 of the 296 councils have no coverage row at all, by design. Benchmark `bulk_create` vs `COPY` at real per-council row counts (275K–415K): not done.
...
- [x] **Step 2 — Populate the English council map.** All 296 English LADs (districts + unitaries, ONS `LAD25_RGN25_EN_LU_v2` lookup) now have a `Council` row (GSS code, name, region) and a fetched boundary; 22 have no R2 spend data yet so render outline-only, same non-interactive treatment proven on the 12 previously-missing London boroughs. The GeoJSON idle-outline bundling from PR #36 is what makes this affordable — one request total (5.1MB raw / 1.6MB gzipped for all 296) regardless of council count. `apps/councils/management/commands/onboard_english_councils.py` is the onboarding command; rerun `scripts/fetch_boundaries.py` (no args) after it to pick up boundaries for whatever it added.
