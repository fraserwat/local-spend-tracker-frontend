...
Turning the 32-borough pilot into all of England. The sibling `local-big-con-nationwide` repo has moved faster than assumed — confirmed live via `list_councils()`, 294 of England's 296 local authority districts now have published curated parquet in R2 (not the 44 this line originally said; re-check before trusting either number next time, it moves fast).
...
- [ ] **Isles of Scilly slug mismatch.** R2 published its curated parquet under `isles-of-scilly` (still hyphenated), but `apps.spend.services.r2.normalize_slug()` only ever converts hyphen → underscore, never the reverse — no Django `Council.slug` can produce a hyphenated result, so no `SLUG_OVERRIDES` entry can bridge this one (see `onboard_english_councils.py`). `reload_from_r2` will 404 against R2 for Isles of Scilly indefinitely as a result, even though the source data exists. Fix needs either a slug-matching exception in `normalize_slug`/the loader, or getting the sibling repo to republish under `isles_of_scilly`. Confirmed via a full reconciliation of all 294 R2-published slugs against the 296 ONS LADs: this is the only council where R2 genuinely has data that the current slug bridge can't reach — the 20 other unmatched R2 slugs (Cambridgeshire, Devon, Essex, Kent, Surrey, etc.) are two-tier county councils, a different geography tier the app doesn't onboard by design, not a bridging gap.
