...
- [ ] **Phase 10 — Deployment & data-refresh runbook.** Dockerfile/gunicorn, managed Postgres with automated backups, documented manual data-refresh runbook, scheduled reconciliation. Also confirm static assets (GeoJSON, `manifest.json`, `all-councils.geojson`) get far-future `Cache-Control` headers in prod (WhiteNoise or CDN) — no explicit config exists yet, so repeat visits currently re-fetch the idle-outline bundle every time; matters more once it's ~300 councils' worth of boundaries instead of 4.
