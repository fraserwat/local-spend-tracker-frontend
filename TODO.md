...
- [ ] **Step 10 — Scale to all 32 London boroughs (pilot completion).** 20/32 loaded (boundary + spend + coverage): Haringey, Barnet, Newham, Redbridge (prior), plus Camden, Hackney, Harrow, Havering, Hounslow, Islington, Merton, Richmond upon Thames, Sutton, Tower Hamlets, Wandsworth, Westminster (this pass). `scripts/reconcile_spend.py` added and passes exactly for all 20. Blocked, not part of this repo's scope to fix:
  - **12 boroughs have no curated parquet in `local-big-con-nationwide` yet**: Barking & Dagenham, Bexley, Croydon, Enfield, Greenwich, Hammersmith & Fulham, Hillingdon, Kensington & Chelsea, Kingston upon Thames, Lambeth, Southwark, Waltham Forest. Their `Council` reference rows and `CouncilCoverage` caveat text (from the sibling repo's README table) are ready to go the moment parquet lands.
...
- [x] **Step 11 — Deployment & data-refresh runbook.** Dockerfile (`uv`-based, `psycopg[binary]` needs no compiler toolchain) + `gunicorn`, deployed to Fly.io (`fly.toml`, region `lhr`) against the already-provisioned Neon Postgres. `/healthz` checks real DB connectivity; `SECURE_PROXY_SSL_HEADER`/`SECURE_REDIRECT_EXEMPT` handle Fly's edge-terminated TLS correctly (health checks bypass the edge entirely, real traffic doesn't). `python manage.py reload_from_r2` is the scheduled refresh: diffs each loaded council's R2 manifest sha256 against its last successful load, reloads only what changed, runs weekly via `.github/workflows/weekly-reload.yml` against the live app over `flyctl ssh console`. Runbook lives in `docs/ARCHITECTURE.md`'s Deployment section.
  ...
  *Verify:* `docker build` + a real container run against live Neon/R2 confirmed `/healthz`, council API, Spend View, and static assets all serve correctly with `DEBUG=False`; `reload_from_r2 --dry-run` reports per-council status (`RELOADED`/`UNCHANGED`/`SKIPPED`/`FAILED`) without writing anything.
