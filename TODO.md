...
- [ ] **Step 10 — Scale to all 32 London boroughs (pilot completion).** Superseded by Step 11's `reload_from_r2`: this is no longer a manual "wait for the sibling repo, then re-run the loader by hand" process — the frontend now pulls directly from R2 on its own, no local checkout of `local-big-con-nationwide` involved anywhere in the path. Re-checked live against R2 on 2026-09-04 (querying `manifest/{slug}.json` for all 32 boroughs directly, not from this stale list below): only **2 boroughs have no curated parquet in R2 at all** — Barking & Dagenham, Enfield. Everything else previously listed as blocked (Bexley, Croydon, Greenwich, Hammersmith & Fulham, Hillingdon, Kensington & Chelsea, Kingston upon Thames, Lambeth, Southwark, Waltham Forest) now has a valid manifest with a real row count. The "4 stale `AMOUNT_GBP_EX_VAT` parquet" claim is also outdated — Brent and Bromley have already loaded successfully through the strict 8-column `EXPECTED_COLUMNS` validator via `reload_from_r2` (a schema mismatch would hard-fail the load, not succeed), confirming their R2 data no longer carries the fabricated column; Ealing and Lewisham have valid manifests too but hadn't been attempted yet as of this note.
  - A manual `weekly-reload.yml` `workflow_dispatch` run triggered 2026-09-04 is backfilling every borough with valid R2 data straight into Neon — in progress as of this note (Barnet, Bexley, Brent, Bromley, Camden landed so far, ~19M rows still queued, estimated ~2 hours total). Once it completes, re-verify against `scripts/reconcile_spend.py` and update this line with the real final count — almost certainly 30/32, not 20/32, since only the 2 genuinely-not-in-R2 boroughs should remain unloaded.
