...
- [ ] **Step 1 — Backend & API scaling.** Reference-data onboarding is bulk now, not one-at-a-time: `onboard_english_councils` pulls GSS code + region for every English LAD straight from ONS Open Geography and diffs against what's already in the DB, replacing the old hand-written-migration-per-borough pattern. Still open: an **ETL backfill** for the 264 councils this added (`reload_from_r2`, unmodified, already knows how to pick each one up — deferred pending a decision on Neon cost/runtime at that volume), and a **generated (not hand-typed) `CouncilCoverage` fixture** (ARCHITECTURE Open risk #7) — 289 of the 296 councils have no coverage row at all, by design, since hand-transcription doesn't scale past the pilot. Benchmark `bulk_create` vs `COPY` at real per-council row counts (275K–415K): not done.
